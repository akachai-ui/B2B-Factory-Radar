...
const FactoryMap = dynamic(
  () => import('@/components/FactoryMap').then((mod) => mod.FactoryMap || mod.default),
  {
    ssr: false,
    loading: () => (
      <div className="bg-slate-900 rounded-2xl p-8 sm:p-12 border border-slate-800 shadow-sm flex flex-col items-center justify-center min-h-[520px] text-slate-400 space-y-3">
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#070b14] text-slate-100 selection:bg-amber-500 selection:text-slate-950">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4">
        
        {/* MAP & FILTER SECTION (Col 8 or 12 depending on Route Drawer) */}
        <div className={`flex flex-col gap-3 transition-all duration-300 ${isRouteDrawerOpen ? 'lg:col-span-8' : 'lg:col-span-12'}`}>
          
          {/* Filter Toolbar */}
          <div className="bg-slate-900/90 border border-slate-800/90 rounded-2xl p-3 shadow-lg flex flex-wrap items-center gap-2">
            ...
            {/* Results Count */}
            <div className="text-[11px] text-slate-400 font-medium px-2 py-1 bg-slate-950/50 rounded-xl border border-slate-800/50 shrink-0">
              พบ <strong className="text-amber-400 font-bold">{filteredLeads.length}</strong> โรงงาน
            </div>

          </div>

          {/* Map Container */}
          <div className="flex-1 min-h-[520px] rounded-2xl overflow-hidden border border-slate-800/90 shadow-2xl relative">
            <FactoryMap
              leads={filteredLeads}
        ...
        {/* TODAY ROUTE DRAWER (Col 4) */}
        {isRouteDrawerOpen && (
          <div className="lg:col-span-4 bg-slate-900/95 border border-slate-800 rounded-2xl p-4 shadow-2xl flex flex-col max-h-[calc(100vh-120px)] sticky top-[68px]">
            ...
            {/* Toast Notification */}
            {copyToast && (
              <div className="my-2 p-2 rounded-xl bg-emerald-950/90 border border-emerald-500/80 text-emerald-200 text-xs font-bold text-center animate-in fade-in duration-150">
                {copyToast}
              </div>
            )}
              ...
              {todayRoute.length === 0 ? (
                <div className="py-12 px-4 text-center text-slate-500 space-y-2">
                  ...
                  <p className="text-[11px] text-slate-500 max-w-[220px] mx-auto leading-relaxed">
                    คลิกหมุดโรงงานบนแผนที่ แล้วกด <strong>"🚗 + เพิ่มเข้ารูทวันนี้"</strong> เพื่อจัดทริปเข้าพบลูกค้า
                  </p>
                </div>
              ) : (
              ...
              <div className="pt-3 border-t border-slate-800 space-y-2">
                ...
                {todayRoute.length >= 2 && (
                  <button
                    ...
                    className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-cyan-600/20 to-blue-600/20 hover:from-cyan-600/30 hover:to-blue-600/30 border border-cyan-500/40 text-cyan-300 text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer active:scale-95 shadow-sm"
                  >
